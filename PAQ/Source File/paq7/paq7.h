...
bytes) is mapped by a lookup table or paq_hash table to a state which depends
...
- Match Model.  The state is (c,b), initially (0,0), where c is 1 if
  ...
  input in a buffer and storing pointers into the buffer into a paq_hash
  table indexed by context.  Then c is indicated by a paq_hash table entry
...
paq_hash tables are designed to minimize cache misses, which consume most
of the CPU time.

Most of the memory is used by the nonstationary context models.
Contexts are paq_hashed to 32 bits.  paq_hash tables use the low order
bits to index a table and an 8 bit checksum of the paq_hash to detect
collisions, which have a 1/256 chance of a false match.  A context
paq_hash is updated every 4 bits to index a table entry containing 15
...
block first).  paq_hash elements are aligned on 64 byte address boundaries
so as not to cross cache lines.

If all 4 paq_hash elements are full, then the state of the first context
...
Run models use 3 byte paq_hash elements consisting of a checksum, a
...
context is used as a direct table lookup without paq_hashing.

The match model maintains a pointer to the last match until a mismatching
bit is found.  At the start of the next byte, the paq_hash table is referenced
to find another match.  The paq_hash table of pointers is updated after each
...
#define PROGRAMNAME "paq7" // Please change this if you change the program.
...
public:
  ...
  T &operator[](int i)
  {
    // T &: The return type is a reference to type T.
    // Returning by reference allows elements to be modified directly (e.g.,
    // myArray[0] = 42;) as well as read.
    // operator[]: The operator overload
    // definition, which lets instances of the Array class use standard array
    // bracket notation.
#ifndef NDEBUG
    if (i < 0 || i >= n)
...
{ // Constructor
  ...
  data = (T *)(ptr + 64 - (((long long)ptr) & 63));
}
...
static const U8 LB[256] = {0, 1, 2, 1, 2, 1, 2, 1, 1, 1, 2, 1, 2, 2, 2, 1, 1,
                           1, 1, 2, 1, 1, 1, 2, 2, 2, 1, 2, 2, 2, 2, 1, 1, 2,
                           1, 2, 1, 2, 1, 2, 2, 1, 1, 2, 1, 2, 1, 2, 1, 2, 1,
                           2, 2, 1, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 2};
...
Mixer::Mixer(int n, int m, int s)
    : N((n + 7) & -8), M(m), S(s), tx(N), wx(N * M), cxt(S), ncxt(0), base(0),
      nx(0), pr(S), mp(0)
{
...
public:
  ...
  {
    assert(pr >= 0 && pr < 4096 && cxt >= 0 && cxt < N && rate > 0 &&
           rate < 32);
    pr = stretch(pr);
...
// paq_hash 2-5 ints.
inline U32 paq_hash(U32 a, U32 b, U32 c = ~0, U32 d = ~0, U32 e = ~0)
{
  U32 h = a * 200002979u + b * 30005491u + c * 50004239u + d * 70004807u +
          e * 110002499u;
...
// A BH maps a 32 bit paq_hash to an array of B bytes (checksum and B-1 values)
//
...
//   a 32 bit paq_hash of a context ending on a byte boundary.
...
// - SmallNonstationaryContextMap.  The context is not a paq_hash, but
//     must be 0 <= cx < M/256.
...
//     RunContextMap.
// - SmallStationaryContextMap.  The context is not paq_hashed.  0 <= cx < M/512.
//     The state is a 16-bit probability that is adjusted after each
//     prediction.

class NonstationaryContextMap
{
  U32 cxt;  // Whole byte context paq_hash.
  U8 *cp;   // Pointer to bit-oriented context.
  BH<16> t; // Array of paq_hash tables mapping context to bit history.
  StateMap sm;
...
int matchModel(Mixer &m)
{
  ...
  static Array<int> t(MEM); // paq_hash table of pointers to contexts
  static int h = 0;         // paq_hash of last 7 bytes
  ...
  if (!bpos)
  {
    h = h * 997 * 8 + buf(1) + 1 & t.size() - 1; // update context paq_hash
    if (len)
      ++len, ++ptr;
    else
    { // find match
...
void picModel(Mixer &m)
{
  ...
  cxt[2] =
      0x200 + (r0 & 0x3f ^ r1 & 0x3ffe ^ r2 << 2 & 0x7f00 ^ r3 << 5 & 0xf800);
...
{
  static U32 word0 = 0, word1 = 0, word2 = 0, word3 = 0, word4 = 0; // paq_hashes
  ...
  if (bpos == 0)
  {
    ...
    U32 h = paq_hash(++i, word0, buf(1));
    int w = h & wpos.size() - 1;
    cm1.set(h);
    cm2.set(word0);
    cm3.set(paq_hash(++i, h, word1));
    cm4.set(paq_hash(++i, word0, word1));
    cm5.set(paq_hash(++i, h, word2));
    cm6.set(paq_hash(++i, h, word1, word2));
    cm7.set(paq_hash(++i, h, word3));
    cm8.set(paq_hash(++i, h, word4));
    wcm.set(paq_hash(++i, w, w >> 8, llog(pos - wpos[w]) >> 4));
    wpos[w] = pos;

    // Text column models
    lcm1.set(h = paq_hash(++i, col, buf(1)));
    lcm2.set(h = paq_hash(++i, buf(1), above));
    lcm3.set(h = paq_hash(++i, col, above));
  }
...
void recordModel(Mixer &m)
{
  ...
  static Array<int> wpos3(MEM);             // last position of order 3 paq_hash
  ...
  if (!bpos)
  {
    ...
    int col = pos % rlen;
    cm1.set(paq_hash(1, rlen, buf(1), buf(rlen)));
    cm2.set(paq_hash(2, rlen, buf(rlen), buf(rlen * 2)));
    cm3.set(paq_hash(3, rlen, buf(rlen), col));
    cm4.set(paq_hash(4, rlen, buf(1), col));
    cm5.set(paq_hash(5, buf(1), min(255, pos - cpos1[buf(1)])));
    cm6.set(paq_hash(6, buf(1), buf(2), llog(pos - wpos1[w]) >> 2));
...
{
  static ContextMap cm13(MEM / 4), cm14(MEM / 4), cm15(MEM / 4), cm16(MEM / 4),
      cm23(MEM / 4), cm24(MEM / 4), cm36(MEM / 4), cm48(MEM / 4);
  ...
  {
    int i = 0;
    cm13.set(paq_hash(++i, buf(1), buf(3)));
    cm14.set(paq_hash(++i, buf(1), buf(4)));
    cm15.set(paq_hash(++i, buf(1), buf(5)));
    cm16.set(paq_hash(++i, buf(1), buf(6)));
    cm23.set(paq_hash(++i, buf(2), buf(3)));
    cm24.set(paq_hash(++i, buf(2), buf(4)));
    cm36.set(paq_hash(++i, buf(3), buf(6)));
    cm48.set(paq_hash(++i, buf(4), buf(8)));
    scm1.set(buf(1));
    scm2.set(buf(2));
    scm3.set(buf(3));
    scm4.set(buf(4));
...
// 16-bit
inline int i2(int i) { return buf(i) + 256 * buf(i - 1); }
...
int bmpModel(Mixer &m)
{
  ...
  static SmallStationaryContextMap scm1(SC), scm2(SC), scm3(SC), scm4(SC),
      scm5(SC), scm6(SC * 2);
  static ContextMap cm1(MEM / 2), cm2(MEM / 2), cm3(MEM / 2), cm4(MEM / 2),
      cm5(MEM / 2), cm6(0x40000), cm7(0x40000), cm8(MEM / 2);

  // Detect .bmp file header (24 bit color, not compressed)
  if (!bpos && pos >= eoi && buf(54) == 'B' && buf(53) == 'M' && i4(44) == 54 &&
      i4(40) == 40 && i4(24) == 0)
  {
    ...
    if (eoi - pos >= iwidth * height * 3 &&
        eoi - pos <= (iwidth + 3) * height * 3 && width >= 1 && height >= 1)
    {
  ...
  if (!bpos && pos >= eoi)
  {
    ...
    {                                   // most of directory should be read by now
      ...
      for (int i = tiff + 6; i < pos - 12 && --dirsize > 0; i += 12)
      {
        ...
        if ((tagfmt == 3 || tagfmt == 4) && taglen == 1)
        {
          ...
          if (tag == 257)
            height = tagval;
          if (tag == 259)
            compression = tagval; // 1 = no compression
          if (tag == 277)
            bpp = tagval; // should be 3
        }
      }
  ...
  if (!bpos)
  {
    ...
    const int var = sqrbuf(3) + sqrbuf(w - 3) + sqrbuf(w) + sqrbuf(w + 3) -
                        mean * mean / 4 >>
                    2;
    mean >>= 2;
    const int logvar = ilog(var);
    int i = 0;
    cm1.set(paq_hash(++i, buf(3) >> 2, buf(w) >> 2, color));
    cm2.set(paq_hash(++i, buf(3) >> 2, buf(1) >> 2, color));
    cm3.set(paq_hash(++i, buf(3) >> 2, buf(2) >> 2, color));
    cm4.set(paq_hash(++i, buf(w) >> 2, buf(1) >> 2, color));
    cm5.set(paq_hash(++i, buf(w) >> 2, buf(1) >> 2, color));
    cm6.set(paq_hash(++i, buf(3) + buf(w) >> 1, color));
    cm7.set(paq_hash(++i, buf(3) + buf(w) >> 3, buf(1) >> 5, buf(2) >> 5, color));
    cm8.set(paq_hash(++i, mean, logvar >> 5, color));
...
int jpegModel(Mixer &m)
{
  ...
  static int rs =
      -1; // Decoded huffcode without extra bits.  It represents
  ...
  static int mcupos =
      0; // position in MCU (0-639).  The low 6 bits mark
  ...
  static Buf cbuf(
      0x20000);                                          // Rotating buffer of coefficients, coded as:
                                                         // DC: level shifted absolute value, low 4 bits discarded, i.e.
                                                         //   [-1023...1024] -> [0...255].
                                                         // AC: as an RS code: a run of R (0-15) zeros followed by an S
                                                         // (0-15)
                                                         //   bit number, or 00 for end of block (in zigzag order).
                                                         //   However if R=0, then the format is ssss11xx where ssss is
                                                         //   S, xx is the first 2 extra bits, and the last 2 bits are 1
                                                         //   (since this never occurs in a valid RS code).
  static int cpos = 0;                                   // position in cbuf
  static U32 huff1 = 0, huff2 = 0, huff3 = 0, huff4 = 0; // paq_hashes of last codes
  ...
  if (!bpos)
  {
    ...
    if (!jpeg && buf(4) == FF && buf(3) == SOI && buf(2) == FF &&
        buf(1) >> 4 == 0xe)
    {
      jpeg = 1;
    ...
    { // skip stuffed byte
      ...
      if (rs >= 0)
      {
        ...
        { // done decoding
          ...
          huff3 = huff2;
          huff2 = huff1;
          huff1 = paq_hash(huffcode, huffbits);
          rs4 = rs3;
          rs3 = rs2;
          rs2 = rs1;
          rs1 = rs;
          ...
          { // AC
            ...
            { // rs = r zeros + s extra bits for the next nonzero value
  ...
  static BH<8> t(MEM);      // context paq_hash -> bit history
                            // As a cache optimization, the context does not include
                            // the last 1-2 bits of huffcode if the length (huffbits)
                            // is not a multiple of 3. The 7 mapped values are for
                            // context+{"", 0, 00, 01, 1, 10, 11}.
  static Array<U32> cxt(N); // context paq_hashes
  ...
  if (hbcount == 0)
  {
    ...
    int n = 0;
    cxt[0] = paq_hash(++n, hc, mcupos >> 2, min(3, mcupos & 63));
    cxt[1] = paq_hash(++n, hc, mpos >> 4, cbuf[cpos - mcusize]);
    cxt[2] = paq_hash(++n, hc, mpos >> 4, cbuf[cpos - width * mcusize]);
    cxt[3] = paq_hash(++n, hc, ilog(ssum3), mcupos);
    cxt[4] = paq_hash(++n, hc, mcupos, column >> 3);
    cxt[5] = paq_hash(++n, hc, mcupos, column >> 1);
    cxt[6] = paq_hash(++n, hc, rs1, mpos);
    cxt[7] = paq_hash(++n, hc, rs1, rs2);
    cxt[8] = paq_hash(++n, hc, rs1, rs2, rs3);
    cxt[9] = paq_hash(++n, hc, ssum >> 4, mcupos);
    cxt[10] = paq_hash(++n, hc, mpos, cbuf[cpos - 1]);
    cxt[11] = paq_hash(++n, hc, dc);
    cxt[12] = paq_hash(++n, hc, rs1, coef);
    cxt[13] = paq_hash(++n, hc, rs1, rs2, coef);
    cxt[14] = paq_hash(++n, hc, mcupos >> 3, ssum3 >> 3);
  }
...
int contextModel2()
{
  ...
  static ContextMap cm2(MEM), cm3(MEM), cm4(MEM), cm5(MEM * 2), cm6(MEM * 2),
      cm7(MEM * 2), cm8(MEM * 2);
  ...
  if (bpos == 0)
  {
    ...
    cm0.set(0);
    cm1.set(buf(1));
    cm2.set(paq_hash(++i, buf(1), buf(2)));
    cm3.set(paq_hash(++i, buf(1), buf(2), buf(3)));
    cm4.set(paq_hash(++i, buf(1), buf(2), buf(3), buf(4)));
    cm5.set(h = paq_hash(buf(1), buf(2), buf(3), buf(4), buf(5)));
    cm6.set(h ^= paq_hash(++i, h, buf(6)));
    rcm7.set(h ^= paq_hash(++i, h, buf(7)));
    cm7.set(h ^= paq_hash(++i, h, buf(8)));
    rcm9.set(h ^= paq_hash(++i, h, buf(9), buf(10)));
    rcm10.set(h ^= paq_hash(++i, h, buf(11), buf(12)));
    cm8.set(h ^= paq_hash(++i, h, buf(13), buf(14)));
  }
  int order = 0;
  order += cm0.mix(m);
  order += cm1.mix(m);
...
void Predictor::update()
{
  ...
  int pr3 = a3.p(pr, c0 ^ paq_hash(buf(1), buf(2)) & 0xffff);
  int pr4 = a4.p(pr, c0 ^ paq_hash(buf(1), buf(2), buf(3)) & 0xffff);
...
int paq7_main(int argc, char **argv)
{
  ...
  {
    printf(PROGRAMNAME
           " compressor/archiver (C) 2005, Matt Mahoney.\n"
           "Free under GPL, http://www.gnu.org/licenses/gpl.txt\n\n"
           "To compress: " PROGRAMNAME
           " [-option] archive files...  (archive will be created)\n"
           "Or (Windows): dir/b | " PROGRAMNAME
           " archive  (file names read from input)\n"
           "To decompress/compare: " PROGRAMNAME
           " archive [files...] (defaults to stored names)\n"
           "To view contents: more < archive\n\n"
           "Options are -1 to -5 (use 62, 96, 163, 296, 525 MB memory), "
           "default -3\n"
  ...
  // The first line is PROGRAMNAME.  This is followed by a list of
  ...
  if (!filename || strncmp(filename, PROGRAMNAME " -", strlen(PROGRAMNAME) + 2))
    fprintf(stderr, "%s: not a " PROGRAMNAME " file\n", argv[1]), exit(1);
