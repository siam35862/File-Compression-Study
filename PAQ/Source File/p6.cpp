 ...
 * Windows Executables
 * P5 - "Small" model discussed in the paper below. Gives the worst compression, but uses the least memory (256K).
 * P6 - "Large" model, gives better compression. Requires 16 MB of memory to compress or decompress, plus memory for Windows and other progams.
 * P12 - Improved version of P6. Gives 3-8% better compression on text files than P6 but about 10% slower, still uses 16 MB. Compiled with Borland 5.5 on 5/13/00.
 * P12a - Compatible with P12 but 9% faster than P12 with a smaller executable (33,792 bytes). Compiled by Jason Schmidt, (schmidtj at tampabay dot rr dot com) 9/6/03 using VS .net 2003 (7.1) with the /O2 (optimize for maximum speed) and /G7 (Pentium 4, AMD Athlon, or higher) options, then packed with UPX --brute --force.
 *
 */

/* P6 - File archiver and compressor.  Matt Mahoney
 *
 * To compress:      P6 archive file file...  (1 or more file names), or
 *  or (MSDOS):     dir/b | P6 archive       (read file names from input)
 *  or (UNIX):      ls    | P6 archive
 * To decompress:    P6 archive
 ...
 * line, then P6 prompts for them, reading until the first blank line or
 * end of file.
 *
 ...
 * which must exist.  P6 does not create directories.  If the file to be
 ...
 * for m (= 5) inputs.  Low values are good for correlated inputs, 1 is
 * optimal for independent inputs.
 *
 * The m inputs are divided into 5 sets for a total of 2^22 = 4,194,304
 ...
 * immediate context, 1-5 previous bytes plus the 0-7 bits of the
 * partial current byte, plus the position (0-7) within the current byte.
 * The context sets are:
 ...
 * - The last 5 bytes plus current bits and position, hashed to a 22 bit number.
 * The 4 hashed context indices share a common space of 2^22 - 2^16 inputs
 * that do not overlap the first set of 2^16 inputs.
 *
 * The parameters were hand tuned on "Alice in Wonderland" to d = 0.5,
 * RL = 0.3, and RS = (0.08, 0.15, 0.2, 0.2, 0.2) for each of the 5 sets
 * above.  These values were found to work well on other text files also.
 */
...
Predictor::Predictor(): pr(32768), s7(0), s3(0), s0(1),
sigma2(0.35) // Set RL here
{
    wt=new Wt[N];
...
void Predictor::update(const int y) {
    ...
    static const int RS[NX]={1024*0.08, 1024*0.15, 1024*0.2,
        1024*0.2,1024*0.2};
        const int CMAX=250;
        for (int i=0; i<NX; ++i) {
            Wt& w=wt[x[i]];
            ...
            w.w+=(error*(RS[i]+sigma2(w.c01)))>>16;
        }
        ...
        ulong s4=s3+s0;          // Last 3 full bytes + 0-7 bits + position in byte

        // Select active inputs (hash x[1..4] modulo primes < 2^22 - 2^16)
        x[0]=4128768+(s4&0xffff);       // Last 1 byte + 0-7 bits + position
        x[1]=(s4&0xffffff)%4128511;     // Last 2 bytes + bits + position
        x[2]=s4%4128493;                // Last 3 bytes + bits + position
        x[3]=(s4+s7*0x3000000)%4128451; // Last 4 bytes + bits + position
        x[4]=(s4+s7*0x50000)%4128427;   // Last 5 bytes + bits + position
...
main(int argc, char** argv) {
    ...
    // Check arguments
    if (argc<2) {
        printf("P6 - File compressor/archiver\n"
        "To compress:  P6 archive filename [filename...]\n"
        "  or (MSDOS): dir/b | P6 archive\n"
        "  or (UNIX):  ls    | P6 archive\n"
        "To extract:   P6 archive\n"
    ...
    if (archive) {
        ...
        // Read "P6\r\n" at start of archive
        if (getline(archive) != "P6") {
            printf("Archive file %s not in P6 format\n", argv[1]);
            return 1;
        }
        ...
        // Test end of header for "\f\0"
        {
            ...
            if ((c1=getc(archive))!='\f' || (c2=getc(archive))!=0) {
                printf("%s: Bad P6 header format %d %d\n", argv[1],
                       c1, c2);
                return 1;
            }
        }
