...
Mixer::Mixer(int n, int m, int s, int w) : N((n + 7) & -8), M(m), S(s), tx(N), wx(N * M),
...
void recordModel(Mixer &m)
{
    ...
    if (!bpos)
    {
        ...
        int w = c4 & 0xffff;
        int r = pos - cpos1[c];
        if (r > 1 && r == cpos1[c] - cpos2[c] && r == cpos2[c] - cpos3[c] && r == cpos3[c] - cpos4[c] && (r > 15 || (c == buf(r * 5 + 1)) && c == buf(r * 6 + 1)))
        {
            if (r == rlen1)
                ++rcount1;
            else if (r == rlen2)
                ++rcount2;
            else if (rcount1 > rcount2)
                rlen2 = r, rcount2 = 1;
            else
                rlen1 = r, rcount1 = 1;
        }
        if (rcount1 > 15 && rlen != rlen1)
            rlen = rlen1, rcount1 = rcount2 = 0;
        if (rcount2 > 15 && rlen != rlen2)
            rlen = rlen2, rcount1 = rcount2 = 0;
...
int bmpModel(Mixer &m)
{
    ...
    if (!bpos && buf(54) == 'B' && buf(53) == 'M' && i4(44) == 54 && i4(40) == 40 && i4(24) == 0)
    {
...
Filetype detect(FILE *in, int n, Filetype type)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (i >= 3 && (buf0 & 0xfffffff0) == 0xffd8ffe0)
            soi = i;
        if (soi && i - soi < 0x10000 && (buf1 & 0xff) == 0xff && (buf0 & 0xff0000ff) == 0xc0000008)
            sof = i;
        if (soi && sof && sof > soi && i - soi < 0x10000 && i - sof < 0x1000 && (buf0 & 0xffff) == 0xffda)
        {
            sos = i;
            if (type != JPEG)
...
int decode_exe(Encoder &en)
{
    ...
    if (q == 5 && (c[4] == 0xe8 || c[4] == 0xe9) && (c[0] == 0 || c[0] == 0xff) && ((offset - 1 ^ offset - 5) & -BLOCK) == 0)
...
int decode(Encoder &en)
{
    ...
    {
    case JPEG:
        return decode_jpeg(en);
    case EXE:
        return decode_exe(en);
    default:
...
void decompress(const char *filename, long filesize, Encoder &en)
{
    ...
    else
    {
        f = fopen(filename, "wb");
        if (!f)
        { // Try creating directories in path and try again
...
int expand(String &archive, String &s, const char *fname, int base)
{
    ...
    else if (sb.st_mode & S_IFDIR && sb.st_mode & 0400 && sb.st_mode & 0100)
    {
        ...
        printf("%s is not a readable file or directory\n", fname);
    return result;
}
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        String archiveName(argv[1]);
        {
            ...
            if (arg1size > prognamesize + 1 && argv[1][arg1size - prognamesize - 1] == '.' && equals(PROGNAME, argv[1] + arg1size - prognamesize))
            {
                mode = DECOMPRESS;
            }
            else if (doExtract)
        ...
        if (mode == DECOMPRESS)
        {
            ...
            for (int i = 0; i < files; ++i)
            {
                ...
                while (*p && *p != '\r')
                    ++p;
                if (!*p)
                    printf("%s: header corrupted at %d\n", archiveName.c_str(),
                           p - &filenames[0]),
                        quit();
