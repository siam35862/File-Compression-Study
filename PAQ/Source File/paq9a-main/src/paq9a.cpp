...
// Initialize
Predictor::Predictor() : c0(0), nibble(1), bcount(0), t(MEM / 2),
...
void list(const char *archive)
{
  ...
  while (true)
  {
    ...
    if (c == EOF)
      break;
    if (c)
    { // start of new file?  Print previous file
      if (mode)
...
int main(int argc, char **argv)
{
  ...
  if (argc < 3 || argv[1][1] || (argv[1][0] != 'a' && argv[1][0] != 'x' && argv[1][0] != 'l') || (argv[1][0] == 'a' && argc < 4) || argv[2][0] == '-')
  {
  ...
  if (argv[1][0] == 'a')
  {
    ...
    if (out)
      printf("Cannot overwrite archive %s\n", argv[2]), exit(1);
    out = fopen(argv[2], "wb");
    if (!out)
      printf("Cannot create archive %s\n", argv[2]), exit(1);
    fprintf(out, "pQ9%c", 1);
    int i = 3;
    if (argc > 3 && argv[3][0] == '-' && argv[3][1] >= '1' && argv[3][1] <= '9' && argv[3][2] == 0)
    {
      putc(argv[3][1], out);
      MEM = 1 << 22 + argv[3][1] - '0';
      ++i;
    ...
    for (; i < argc; ++i)
    {
      if (argv[i][0] == '-' && (argv[i][1] == 'c' || argv[i][1] == 's') && argv[i][2] == 0)
        option = argv[i][1];
      else
