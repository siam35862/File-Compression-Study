...
static inline bool isalpha_cpu(char ch)
{
    return (ch >= 'A' && ch <= 'Z') ||
           (ch >= 'a' && ch <= 'z');
}
...
LZP::LZP(StateMap *statemap, U8 *buf, U32 *tab, APM *a1, APM *a2, APM *a3)
    : N(g_MEM / 8), H(g_MEM / 32),
      match(-1), len(0), pos(0), hash(0), hash1(0), hash2(0),
      statemap(statemap), apm1(a1), apm2(a2), apm3(a3),
...
Predictor::Predictor(U8 *context1_ptr, StateMap *statemap1[N], Mix *mix1[N - 1], APM *a1, APM *a2, APM *a3, HashTable<16> *hashtable_ptr, LZP *lzp_ptr)
    : c0(0), nibble(1), bcount(0), hashtable(hashtable_ptr),
      apm1(a1), apm2(a2), apm3(a3), context1(context1_ptr), lzp(lzp_ptr)
{
    for (int i = 0; i < N; ++i)
    {
        sp[i] = cp[i] = context1;
...
Encoder::Encoder(int m, char *temp, unsigned char *buffer_ptr, size_t tsz, size_t itr, Predictor *pr)
    : mode(m), inout(temp), total_size(tsz), pred(pr), x1(0), x2(0xffffffff), x(0),
      buffer(buffer_ptr), usize(0), csize(0), usum(0), csum(0), iterator_size(itr)
{
    if (mode == DECOMPRESS)
...
static void compressChunk(const char *input, size_t input_size, char *output, size_t &output_size, ChunkBuffers &b)
{
    ...
    Predictor pred(b.predictor_context1.data(), sm_ptrs, mix_ptrs, &predictor_apm1, &predictor_apm2, &predictor_apm3, &hashtable, &lzp);
...
void compress(const char *destination_file, const char *source_file)
{
    ...
    std::cout << "Total: " << total_uncompressed_size.load() << " Byte -> " << total_compressed_size.load() << " Byte" << endl;
    std::cout << "Compression Ratio: " << 1.0 * (double)total_uncompressed_size.load() / (double)std::max<size_t>(1, total_compressed_size.load()) << endl;
}
...
void decompress(const char *destination_file, const char *source_file)
{
    ...
    else if (mode == 'c')
    {
        ...
        std::cout << "Total: " << total_compressed_size.load() << " Byte -> "
                  << total_uncompressed_size.load() << " Byte" << endl;
    }
}
...
int main(int argc, char **args)
{
    ...
    if (seconds > 0)
        std::cout << "Compression/Decompression Speed: " << (double)total_uncompressed_size.load() / seconds / 1024 << " KB/seconds \n";
